...
from delsmm.smm import ForcedSMM, StructuredMechanicalModel
...
def run(seed, lr, method, noise, damped, smoketest=False):
    ...
    dt = 0.05
    logdir = "data/%s_%s_%.1e_%.3f_%d" % (
        "damped" if damped else "undamped",
        method,
        lr,
        noise,
        seed,
    ...
    sched = torch.optim.lr_scheduler.LambdaLR(opt, lambda k: 500.0 / (500 + k))
...
@click.option("-n",
              ...
              type=float,
              default=0.1,
              help="Noise setting for the experiment")
@click.option("--parallel",
              type=int,
              default=0,
              help="Set > 1 for number of parallel jobs to run")
def main(noise, parallel):
    ...
    if parallel > 1:
        ...
        Parallel(n_jobs=parallel)(delayed(run)(seed, lr, method, noise, damped)
                                  for method in methods for lr in lrs
