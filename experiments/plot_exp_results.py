...
def main():
            ...
            for seed_ in range(105):
                for method_ in ["del+logdet", "qdd", "nqqd"]:
                    ...
                    logdir = "data/%s_%s_%.1e_%.3f_%d" % (
                        "damped" if damped_ else "undamped",
                        method_,
                        lr_,
                        noise_,
                        seed_,
                    ...
                    try:
                        ...
                        error.append(df["eval/best_val_loss_test_qddot_loss"].
                                     values[-1])
                        noise.append(noise_)
    ...
    df_ = df.groupby(["damped", "noise", "method", "lr"]).agg(
        ...
        std_error=pd.NamedAgg(column="error",
                              aggfunc=lambda x: np.std(x) / np.sqrt(len(x))),
    )

    df_mm = (df.loc[df.lr == 0.0001].groupby("seed").agg(
        minmax_error=pd.NamedAgg(column="error", aggfunc=np.max)))
    ...
    for damped in ["damped", "undamped"]:
        ...
        plt.bar(xs,
                df__.mean_error.values,
                yerr=df__.std_error.values,
                label="DEL-residual")
